...
public sealed class FournisseurEventConsumer : BackgroundService
{
        ...
        ILogger<FournisseurEventConsumer> logger)
    {
        ...
        var config = new ConsumerConfig
        {
            ...
            GroupId = "fournisseur-service-fournisseur-cache",
        ...
        _consumer.Subscribe([FrournisseurTopics.Created, FrournisseurTopics.Updated, FrournisseurTopics.Deleted, FrournisseurTopics.Restored]);
    }

    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        _logger.LogInformation("FournisseurEventConsumer started, topics: {Topics}",
            string.Join(", ", FrournisseurTopics.Created, FrournisseurTopics.Updated, FrournisseurTopics.Deleted, FrournisseurTopics.Restored));
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    using (var scope = _scopeFactory.CreateScope())
                    {
                        ...
                        switch (result.Topic)
                        {
                            case FrournisseurTopics.Created:
                                await handler.HandleCreatedAsync(dto);
                                break;
                            case FrournisseurTopics.Updated:
                                await handler.HandleUpdatedAsync(dto);
                                break;
                            case FrournisseurTopics.Deleted:
                                await handler.HandleDeletedAsync(dto);
                                break;
                            case FrournisseurTopics.Restored:
