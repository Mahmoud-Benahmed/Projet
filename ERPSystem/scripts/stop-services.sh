...
CONTAINERS=(erp-kafka erp-gateway erp-auth erp-auth-mongo erp-article erp-article-sqlserver erp-client)
...
if [ "$DELETE" = true ]; then
  ...
  echo -e "${BLUE}Stopping containers (use --delete or -d to remove them)...${NC}"
  for container in "${CONTAINERS[@]}"; do
  ...
  echo -e "${GREEN}Done stopping containers.${NC}"
fi
